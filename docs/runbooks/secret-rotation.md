...
- An authenticated `/metrics` request made with the separately rotated global
  ...
  non-`degraded` state.
- Manual runtime/SIEM notification delivery smoke tests succeed. New scheduler deliveries
  resolve the rotated destination by keyed fingerprint; pre-rotation pending rows become
  `blocked` rather than sending to a different destination.
